...
**Versions 2025.0.3 and 2025.0.4** introduce the Docker Swarm directory architecture, that will allow for scalability and queues in runs. A `drugapp-launcher.sh` script will be added. It will allow to either build the docker images and launch the containers (*i.e.* ```druapp-launcher.sh --build```) or remove them (*i.e.* ```drugapp-launcher.sh --remove```). Such command has to be executable beforehand with ```chmod +x drugapp-launcher.sh```. The current version runs the Flask App, yet the data analysis steps are currently on hold to delocalise them on sub-services: the old version up to 2025-0-2 is available at in the **back-2025.0.2 branch**.
