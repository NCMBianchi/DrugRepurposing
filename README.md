...
**Version 2025.0.3** implements the first steps for a Docker Swarm structure, allowing for scalability and queues in runs. It's currently still being finalised. Moreover, a `drugapp-launcher.sh` script has been added. It allows to either build the docker images and launch the containers (*i.e.* ```druapp-launcher.sh --build```) or remove them (*i.e.* ```drugapp-launcher.sh --remove```). Such command has to be executable beforehand with ```chmod +x drugapp-launcher.sh```.
