...
def monarch_service_run():
    """
    Flask route to run Monarch service for distributed architecture
    """
    ...
    def setup_monarch_globals(today_directory, date_str, input_seed):
    ...
    # Setup globals
    today_directory, date_str, input_seed, input_file_path = setup_monarch_globals(
        today_directory, date_str, input_seed
    )
    
    try:
        # Call the existing run_monarch function
        ...
        response = {
            ...
            'disease_name_label': disease_name_label,
            'disease_directories': disease_directories,
            # Include any additional payload data that might be needed
            **{k: v for k, v in payload.items() if k not in ['input_seed', 'today']}
        }

        return jsonify(response)

    except Exception as e:
        logging.error(f"Error in Monarch service: {str(e)}")
        return jsonify({
            'error': str(e),
            'status': 'failed'
        }), 500
