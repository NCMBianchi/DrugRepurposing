"""
NEGATIVE SAMPLES MODULE FOR DISTRIBUTED SERVICES
...
# Import the core Negative Samples functionality
from .negsamples_legacy import run_negsamples as legacy_run_negsamples
...
def run_negsamples(
    ...
    **kwargs
    ...
    """
    Wrapper for legacy_run_negsamples that adapts to microservice environment
    ...
    :return: Tuple of negative sample nodes and edges
    """
    ...
    today_directory, date_str, input_seed, input_file_path = setup_service_globals(
        base_directory, date, monarch_input, 'negsamples'
    )

    # Run the legacy function
    return legacy_run_negsamples(monarch_input, date, **kwargs)

def process_service_request(env_vars: Dict[str, str]) -> Dict[str, Any]:
    """
    Process Negative Samples service request based on environment variables
    ...
    try:
        # Run Negative Samples generation
        negative_nodes, negative_edges = run_negsamples(
        ...
        logging.error(f"Negative Samples service processing failed: {e}")
        return {
            'status': 'error',
            'message': str(e)
        }
